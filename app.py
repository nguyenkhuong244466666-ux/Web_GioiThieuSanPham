...
def admin_upload():
    db = next(get_db())
    try:
        categories = db.query(CategoryModel).filter(CategoryModel.slug.notin_(['cua-so', 'tu'])).all()
        if request.method == 'POST':
            category_id = request.form.get('category_id')
            title = request.form.get('title', '').strip()
            files = request.files.getlist('images')
            if not category_id or not files:
                flash('Vui lòng chọn danh mục và ảnh!', 'warning')
            ...
            for file in files:
                if file and allowed_file(file.filename):
                    ...
                    file.save(file_path)
                    new_product = ProductModel(category_id=category.id, title=title or category.name, image=unique_filename)
                    db.add(new_product)
                    uploaded_count += 1
            db.commit()
            flash(f'Tải lên thành công {uploaded_count} ảnh!', 'success')
